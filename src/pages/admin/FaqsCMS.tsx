  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-bold text-gray-800 dark:text-foreground">Manage FAQs</h1>
          <p className="text-muted-foreground mt-1">Add, edit, or remove Frequently Asked Questions.</p>
        </div>
          ...
          <table className="w-full text-left text-sm text-muted-foreground dark:text-muted-foreground">
            ...
            <tbody>
              {loading ? (
                ...
                faqs.map((faq) => (
                  <tr key={faq._id} className="border-b border-zinc-200 dark:border-border hover:bg-gray-50 dark:hover:bg-accent/30">
                    <td className="px-6 py-4 font-medium text-gray-900 dark:text-foreground">{faq.order}</td>
                    ...
                    <td className="px-6 py-4"><span className="px-3 py-1 bg-zinc-100 dark:bg-accent rounded-full text-xs font-medium">{faq.category}</span></td>
        ...
        <div className="fixed inset-0 bg-background/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-card rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto border border-zinc-200 dark:border-border shadow-2xl">
            <div className="flex justify-between items-center p-6 border-b border-zinc-200 dark:border-border sticky top-0 bg-white dark:bg-card z-10">
              <h2 className="text-2xl font-bold dark:text-foreground">{editingId ? 'Edit FAQ' : 'New FAQ'}</h2>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="md:col-span-2">
                  <label className="block text-sm font-medium mb-2 dark:text-foreground/80">Question</label>
                  <input required type="text" name="question" value={formData.question} onChange={handleInputChange} className="w-full bg-transparent border border-zinc-300 dark:border-border rounded-lg px-4 py-3 dark:text-foreground focus:border-primary outline-none" />
                </div>
                <div>
                  <label className="block text-sm font-medium mb-2 dark:text-foreground/80">Category</label>
                  <input required type="text" name="category" value={formData.category} onChange={handleInputChange} placeholder="e.g. General, Services, Pricing" className="w-full bg-transparent border border-zinc-300 dark:border-border rounded-lg px-4 py-3 dark:text-foreground focus:border-primary outline-none" />
                </div>
                <div>
              ...
              <div>
                <label className="block text-sm font-medium mb-2 dark:text-foreground/80">Answer</label>
                <textarea rows={5} required name="answer" value={formData.answer} onChange={handleInputChange} className="w-full bg-transparent border border-zinc-300 dark:border-border rounded-lg px-4 py-3 dark:text-foreground focus:border-primary outline-none"></textarea>
              </div>
              ...
              <div className="flex justify-end gap-4 pt-6 border-t border-zinc-200 dark:border-border">
                ...
                <button type="submit" className="px-6 py-3 font-medium rounded-lg bg-primary text-foreground hover:bg-primary/90">Save FAQ</button>
              </div>
            </form>
          </div>
        </div>
