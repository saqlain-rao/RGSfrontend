  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-bold text-gray-800 dark:text-white">Inbox</h1>
          <p className="text-gray-500 mt-1">View messages submitted from the contact form.</p>
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Messages List */}
        <div className="lg:col-span-1 bg-white dark:bg-zinc-900 rounded-xl shadow-sm border border-zinc-200 dark:border-zinc-800 overflow-hidden flex flex-col h-[75vh]">
          <div className="p-4 border-b border-zinc-200 dark:border-zinc-800 bg-gray-50 dark:bg-zinc-800/50">
            <h2 className="font-semibold text-gray-700 dark:text-gray-300">All Messages ({messages.length})</h2>
          </div>
              ...
              <ul className="divide-y divide-zinc-200 dark:divide-zinc-800">
                {messages.map((msg) => (
                  <li 
                    ...
                    className={`p-4 cursor-pointer hover:bg-gray-50 dark:hover:bg-zinc-800/50 transition-colors ${selectedMessage?._id === msg._id ? 'bg-blue-50 dark:bg-zinc-800 border-l-4 border-primary' : 'border-l-4 border-transparent'}`}
                  >
                    <div className="flex justify-between items-start mb-1">
                      <h3 className={`font-medium ${msg.status === 'Unread' ? 'text-gray-900 dark:text-white font-bold' : 'text-gray-700 dark:text-gray-300'}`}>
                        {msg.name}
                      </h3>
                    ...
                    <p className={`text-sm truncate mb-2 ${msg.status === 'Unread' ? 'text-gray-800 dark:text-gray-200 font-medium' : 'text-gray-500 dark:text-gray-400'}`}>
                      {msg.subject}
                    </p>
        ...
        {/* Message Detail View */}
        <div className="lg:col-span-2 bg-white dark:bg-zinc-900 rounded-xl shadow-sm border border-zinc-200 dark:border-zinc-800 h-[75vh] flex flex-col">
          {selectedMessage ? (
            <>
              <div className="p-6 border-b border-zinc-200 dark:border-zinc-800 flex justify-between items-start">
                <div>
                  <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-4">{selectedMessage.subject}</h2>
              ...
              <div className="p-6 flex-1 overflow-y-auto">
                <div className="bg-gray-50 dark:bg-zinc-800/30 p-6 rounded-lg whitespace-pre-wrap text-gray-800 dark:text-gray-200 leading-relaxed font-medium">
                  {selectedMessage.message}
                </div>
              </div>
              
              <div className="p-4 border-t border-zinc-200 dark:border-zinc-800 bg-gray-50 dark:bg-zinc-800/50 flex justify-end">
                <a 
