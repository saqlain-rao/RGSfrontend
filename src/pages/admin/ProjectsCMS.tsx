  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-bold text-gray-800 dark:text-foreground">Manage Projects</h1>
          <p className="text-muted-foreground mt-1">Add, edit, or remove construction projects from the portfolio.</p>
        </div>
            ...
            <tbody>
              {loading ? (
                ...
                projects.map((project) => (
                  <tr key={project._id} className="border-b border-zinc-200 dark:border-border hover:bg-gray-50 dark:hover:bg-accent/30">
                    <td className="px-6 py-4">
                      {project.mainImage ? (
                        <img src={project.mainImage} alt={project.title} className="w-16 h-12 object-cover rounded-md border border-zinc-200 dark:border-border" />
                      ) : (
                    ...
                    <td className="px-6 py-4 font-medium text-gray-900 dark:text-foreground">{project.title}</td>
                    <td className="px-6 py-4"><span className="px-3 py-1 bg-zinc-100 dark:bg-accent rounded-full text-xs font-medium">{project.category}</span></td>
                    <td className="px-6 py-4">{project.featured ? <span className="text-green-500 flex items-center gap-1"><CheckCircle size={14}/> Featured</span> : <span className="text-muted-foreground">Standard</span>}</td>
      ...
      {modalOpen && (
        <div className="fixed inset-0 bg-background/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-card rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto border border-zinc-200 dark:border-border shadow-2xl">
            <div className="flex justify-between items-center p-6 border-b border-zinc-200 dark:border-border sticky top-0 bg-white dark:bg-card z-10">
              <h2 className="text-2xl font-bold dark:text-foreground">{editingId ? 'Edit Project' : 'New Project'}</h2>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-sm font-medium mb-2 dark:text-foreground/80">Project Title</label>
                ...
                <div>
                  <label className="block text-sm font-medium mb-2 dark:text-foreground/80">Category</label>
                  <select name="category" value={formData.category} onChange={handleInputChange} className="w-full bg-transparent border border-zinc-300 dark:border-border rounded-lg px-4 py-3 dark:text-foreground focus:border-primary outline-none">
                ...
                <div>
                  <label className="block text-sm font-medium mb-2 dark:text-foreground/80">Status</label>
                  <select name="status" value={formData.status} onChange={handleInputChange} className="w-full bg-transparent border border-zinc-300 dark:border-border rounded-lg px-4 py-3 dark:text-foreground focus:border-primary outline-none">
                    <option value="Completed">Completed</option>
                    <option value="Ongoing">Ongoing</option>
                  </select>
                </div>
                <div>
                  <label className="block text-sm font-medium mb-2 dark:text-foreground/80">Client / Investor</label>
                  <input required type="text" name="client" value={formData.client} onChange={handleInputChange} className="w-full bg-transparent border border-zinc-300 dark:border-border rounded-lg px-4 py-3 dark:text-foreground focus:border-primary outline-none" />
                </div>
                <div>
                  <label className="block text-sm font-medium mb-2 dark:text-foreground/80">Location</label>
                  <input required type="text" name="location" value={formData.location} onChange={handleInputChange} className="w-full bg-transparent border border-zinc-300 dark:border-border rounded-lg px-4 py-3 dark:text-foreground focus:border-primary outline-none" />
                </div>
                <div>
                  <label className="block text-sm font-medium mb-2 dark:text-foreground/80">Project Duration</label>
                  <input required type="text" name="duration" value={formData.duration} onChange={handleInputChange} placeholder="e.g. 24 Months" className="w-full bg-transparent border border-zinc-300 dark:border-border rounded-lg px-4 py-3 dark:text-foreground focus:border-primary outline-none" />
                </div>
              </div>
              ...
              <div>
                <label className="block text-sm font-medium mb-2 dark:text-foreground/80">Feature Image (Upload)</label>
                <div className="flex items-center gap-4">
                  {formData.mainImage && <img src={formData.mainImage} alt="Preview" className="w-24 h-24 object-cover rounded-lg border border-border" />}
              ...
              <div className="flex items-center gap-3 bg-zinc-50 dark:bg-accent/50 p-4 rounded-lg border border-zinc-200 dark:border-border">
                ...
                <label htmlFor="featured" className="text-sm font-medium dark:text-foreground cursor-pointer">Mark as Featured Project (Shows on Homepage)</label>
              </div>
              ...
              <div className="flex justify-end gap-4 pt-6 border-t border-zinc-200 dark:border-border">
                ...
                <button type="submit" className="px-6 py-3 font-medium rounded-lg bg-primary text-foreground hover:bg-primary/90">Save Project</button>
              </div>
            </form>
          </div>
        </div>
